...
test.describe('Runbook Admin Basic E2E', () => {
  test('should load homepage', async ({ page }) => {
    ...
    await expect(
      page.getByText('Local GUI for running and managing Runn runbooks'),
    ).toBeVisible()
  })
  ...
  test('should navigate to history page', async ({ page }) => {
    ...
    await expect(
      page.getByText('View all runbook execution results'),
    ).toBeVisible()
  })
  ...
  test('should display current project path', async ({ page }) => {
    ...
    await expect(
      page.locator('p.text-slate-500').filter({ hasText: /📁.*runbook-admin/ }),
    ).toBeVisible()
  })
})
